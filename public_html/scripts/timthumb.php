...
//if(preg_match("/\.gif/",$_GET['src']) && ($_GET['w'] > 400 || $_GET['w']=="") && ($_GET['q'] > 85 || $_GET['q']=="")){
if(preg_match("/\.gif/")){
        $_GET['w'] = 600;
        $_GET['q'] = 100;
}
...
if(! defined('FILE_CACHE_PREFIX') ){
	if($user_agent == 'desktop'){
		...
		//$_GET['q'] = 100;
		//$_GET['w'] = 600;
                define ('FILE_CACHE_PREFIX', 'timthumb_mobi');    // What to put at the beg of all files in the cache directory so we can identify them
        }
}
if(! defined('FILE_CACHE_DIRECTORY') ){
	...
	if($user_agent == 'desktop'){
		...
		define ('FILE_CACHE_DIRECTORY', $secure_folder_path.'/all_cache/timthumb_cache_mobile'); // Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
	}
}
if(! defined('MAX_FILE_SIZE') )		
	define ('MAX_FILE_SIZE', 5242880);	// 10 Megs is 10485760. This is the max internal or external file size that we'll process.  
...
if(! defined('WAIT_BETWEEN_FETCH_ERRORS') )	
	define ('WAIT_BETWEEN_FETCH_ERRORS', 60);				//Time to wait between errors fetching remote file
...
if(! defined('DEFAULT_Q') )		
	define ('DEFAULT_Q', 70);				// Default image quality. Allows overrid in timthumb-config.php
if(! defined('DEFAULT_ZC') )
	define ('DEFAULT_ZC', 0);				// Default zoom/crop setting. Allows overrid in timthumb-config.php
...
class timthumb {
	...
	protected function processImageAndWriteToCache($localImage){
		...
		/* Added By Jagannath Behera : for compressing image less than the original size */
		/*if($_REQUEST['w'] >= 401 && $_REQUEST['q'] >= 95){
			$orig_path = $this->param('src');
			$imgType = "";
			...
			$fp = fopen($root_path.$orig_path,'r',0,$context);
	...
	protected function calcDocRoot(){
		...
		//if($_REQUEST['test'] == '420'){
		if(preg_match('@\/img\/gadget-finder\/@',$_SERVER['REQUEST_URI']) || preg_match('@\/img\/best\/@',$_SERVER['REQUEST_URI']))
			$docRoot = dirname($docRoot).'/public_html_secure/';
		//}
...
function getUserAgent() {
	$userAgent = strtolower($_SERVER['HTTP_X_UA_DEVICE']);
	if($userAgent=="opera")
		return 'mobile';
	if(!empty($userAgent))
		return $userAgent;
